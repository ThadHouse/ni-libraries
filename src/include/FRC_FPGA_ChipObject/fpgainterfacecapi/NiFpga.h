/*
 * FPGA Interface C API 25.8 header file.
 *
 * Copyright (c) 2025,
...
/**
 * The bitfile you are trying to use is too old to use with the firmware
 * installed on the FPGA target.  Ensure your firmware loaded on the device
 * matches the version of software you are using to compile your bitfile.
 */
static const NiFpga_Status NiFpga_Status_BitfileOlderThanFirmware = -63108;

/**
 * The bitfile you are trying to use is too new to use with the firmware
 * installed on the FPGA target.  Ensure your firmware loaded on the device
 * matches the version of software you are using to compile your bitfile.
 */
static const NiFpga_Status NiFpga_Status_BitfileNewerThanFirmware = -63109;
...
/**
 * The provided memory was too fragmented for this device to use it
 * successfully.  Try using a smaller buffer, a more physically contiguous
 * buffer, or less DMA operations at the same time.
 */
static const NiFpga_Status NiFpga_Status_MemoryTooFragmented = -63202;
...
#define NiFpga_Private_FxpToFloatingPoint(typeInfo, delta, data) \
   ...
   { \
      ...
      if (data & signedMask) \
      { \
         int64_t signedData = -(int64_t)((data ^ wordLengthMask) + 1); \
...
#define NiFpga_Private_FloatingPointToFxp(typeInfo, delta, data) \
   ...
   { \
      ...
      { \
         ...
         const int64_t minValue = -(1LL << (typeInfo.wordLength - 1)); \
         if (fxpRepresentation < minValue) \
         { \
            return minValue & wordLengthMask; \
         } \
         else \
         { \
            return ((uint64_t)fxpRepresentation) & wordLengthMask; \
         } \
      } \
      else \
      { \
...
typedef enum
{
    ...
    * Implementation
    * Type: I32
    * The specific implementation of the FIFO, see NiFpga_FifoImplementation.
    */
   NiFpga_FifoProperty_Implementation = 11
} NiFpga_FifoProperty;
...
typedef enum
{
   NiFpga_FifoImplementation_TargetToHost = 0,
   NiFpga_FifoImplementation_HostToTarget = 1,
   NiFpga_FifoImplementation_PeerToPeerReader = 2,
   NiFpga_FifoImplementation_PeerToPeerWriter = 3,
   NiFpga_FifoImplementation_P2PDMAProviderReader = 4,
   NiFpga_FifoImplementation_P2PDMAProviderWriter = 5
} NiFpga_FifoImplementation;
